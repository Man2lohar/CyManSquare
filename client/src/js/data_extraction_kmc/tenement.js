...
function filterTenementData(csv, commonAreaData) {
    ...
    const linetypeIndex = headers.indexOf('Linetype');
    const areaIndex = headers.indexOf('Area');
    const tenementRangeIndex = headers.indexOf('Tenement Range');
    ...
    floorGroup.forEach((entries, floor) => {
        ...
        entries.forEach((entry, index) => {
            const flatName = String.fromCharCode(65 + index) + floor.slice(-1);
            filteredData.push([
                entry.floor,
                flatName,
                entry.tenementType,
...
sortedEntries.forEach((entry, index) => {
    ...
    if (isLastRowOfRange && ['BELOW 50', '50 - 75', '75 - 100'].includes(entry.tenementRange)) {
        // Count the reqCarParking for the last row
        totalReqCarParking += entry.reqCarParking;
        // Calculate or retrieve totalReqCarParking dynamically
localStorage.setItem('totalReqCarParking', totalReqCarParking);
console.log('Total Required Car Parking:', totalReqCarParking);
    }
});
