...
function createTableForLayer(layer, layerData) {
    ...
    footerRow.innerHTML = `
        <td>Nos. of Car Parking Required:</td>
        <td colspan="2">${parkingRequired}</td>
    `;
...
function addTotalParkingRow(totalParkingRequired) {
    ...
    totalRow.classList.add('total-row');
    totalRow.innerHTML = `
        <td style="width: 57.4%;">Total Nos. of Car Parking Required:</td>
        <td colspan="2">${totalParkingRequired}</td>
    `;
    tbody.appendChild(totalRow);

    console.log('Stored totalParkingRequired:', totalParkingRequired);
